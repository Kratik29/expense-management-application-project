  ...
  useEffect(() => {
    const getAllTransactions = async () => {
      try {
        ...
        setAllTransection(res.data.toReversed());
        setLoading(false);
      } catch (error) {
    ...
    try {
      ...
      if (editable) {
        await axios.post("http://localhost:8080/api/v1/transections/edit-transection", {
          payload: {...values,userId: user._id,},transacationId: editable._id,});
        setLoading(false);
        setRefresh(!refresh)
  ...
  return (
    <Layout>
      ...
      <div className="filters">
        ...
        {/*expense type filter ends*/}

        {/*data analytics and expense-table filter*/}
        <div className="switch-icons align-item:center">
        ...
        {/*data analytics and expense table filter ends*/}

         {/* add new expense into the table*/}
        <div>
          <button className="btn btn-primary" onClick={() => setShowModal(true)}>
            ADD NEW </button>
        </div>
        {/* add new expense into the table ends*/}

      </div>
      {/* all filter ends here*/}

       {/* showing the content of table and  */}
      <div className="content">
        {viewData === "table" ? (<Table columns={columns} dataSource={allTransection} />) 
        : (<Analytics allTransection={allTransection} />)}
      </div>
      {/* showing the content of table as */}
